...
error_reporting(0);
$SETTINGS["data_table"]='tblevent_calendar';
...
$sql = "SELECT * FROM ".$SETTINGS["data_table"]." WHERE `event_date` LIKE '".$cYear."-".sprintf('%02d',$cMonth)."-%'";
$sql_result = mysql_query ($sql) or die ('request "Could not execute SQL query" '.$sql. "ERROR: ".mysql_error());
...
// set css class name based on number of events for that day
	if ($events[$cYear."-".sprintf('%02d',$cMonth)."-".$current_day]<>'') {
		$css='withevent';
		$click = "onclick=\"LoadEvents('".$cYear."-".sprintf('%02d',$cMonth)."-".$current_day."')\"";
	} else {
		$css='noevent'; 		
		$click = '';
	}
